...
The current `0.2.35` dogfood shifted from recall display to operational trust. Restarting the local Runtime/Codex integration proved the snapshot path works across process boundaries, but also exposed three practical failure modes: stale pids from failed launchers, hook processes losing the fixed local Runtime command, and automatic tool-feedback telemetry hanging Runtime. The first two were fixed in source; the explicit tools-feedback route is repaired after `0.2.34`, and automatic hook telemetry shipped in `0.2.35` with a separate short timeout budget. A live direct route soak returned `200` in 185ms, and a source `PostToolUse` hook returned normal outcome context in about 105ms. The feedback path now also suppresses generic automatic pattern anchors to avoid a new "prefer exec/bash" memory pollution class.
...
After publishing `0.2.35`, the release path closed cleanly: npm latest returned `0.2.35`, fresh npm exec returned `0.2.35`, fresh install plus audit found zero vulnerabilities, Codex status passed, runtime release baseline passed, and `codex audit` reports the `0.2.35` release outcome with `context_quality=pass`.
