...
| Authority decision reporting | `src/memory/authority-decision-report.ts` | Read-only Trust Gate explanation | Existing gate outputs and source-owned `authority_rules` | Product/debug report only; no authority grant, memory mutation, or promotion |
