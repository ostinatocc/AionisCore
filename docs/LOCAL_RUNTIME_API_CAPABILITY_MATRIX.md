...
`/v1/memory/action/retrieval` and `/v1/memory/tools/select` now reject undeclared top-level route fields. `action_retrieval_v1` also rejects undeclared fields in `rationale`, `tool`, `path`, `evidence`, `evidence.entries[]`, `uncertainty`, and the downstream `action_retrieval_gate` subcontracts; the route now exposes declared workflow, pattern, lifecycle, and authority fields instead of relying on passthrough metadata. `tools_select` exposes strict selection, execution-kernel, rules, decision, pattern-match, and selection-summary subcontracts while pattern anchors remain a declared legacy-storage compatibility boundary.
