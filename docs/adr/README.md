# ADR Archive

`docs/adr` stores architecture decision records for `Aionis Core`.

These records remain useful as design history, but they are not the canonical public definition of `Aionis Core`.

## Needs Retitle

1. [ADR-0001-lite-execution-memory-kernel.md](ADR-0001-lite-execution-memory-kernel.md)

## Historical Archive

1. [ADR-0002-lite-execution-policy-intervention-model.md](ADR-0002-lite-execution-policy-intervention-model.md)
