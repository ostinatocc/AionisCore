...
1. Add a compact dogfood summary command or report now that at least seven real tasks have observations.
2. Make local-runtime dogfooding less confusing by documenting or wiring an explicit workspace runtime command for Codex watchdog testing.
3. Use the next live Codex prompt to check whether `Fast Task Facts` now surfaces the 7/10 state cleanly from the published Runtime.
