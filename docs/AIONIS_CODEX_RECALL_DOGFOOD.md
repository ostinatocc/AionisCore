...
| 8 | Install from npm in a fresh consumer workspace. | Is the first-run path understandable without repo internals? | next |
...
1. Run Task 8 by installing from npm in a fresh consumer workspace and checking the first-run path without repo internals.
2. Publish and verify the `0.2.31` candidate if the user wants the Stop-hook pollution fix and Task 7 coverage in npm.
3. Continue second-repo work after Task 8 so recall quality is tested outside release-only workflows.
