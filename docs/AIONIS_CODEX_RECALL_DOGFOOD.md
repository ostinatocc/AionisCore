...
| 7 | Force slow Runtime find behavior while snapshots exist. | Does task start remain fast and useful under Runtime latency? | next |
...
1. Run Task 7 by forcing slow Runtime find behavior while local snapshots exist, and verify task start stays fast and useful.
2. Publish and verify the `0.2.31` candidate if the user wants this Stop-hook pollution fix in npm.
3. Continue second-repo work after Task 7 so recall quality is tested outside release-only workflows.
