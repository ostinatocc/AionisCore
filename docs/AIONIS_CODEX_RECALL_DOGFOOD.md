...
| 4 | Interrupt and resume a real bug fix across turns. | Does handoff recovery preserve the actual next step? | next |
...
1. Run Task 4 as an interrupted/resumed real bug fix and verify whether Aionis recovers the exact next step without relying on chat history.
2. Move Task 6 earlier if more conceptual/status replies appear in `latest_task_handoff`; this is now the most likely context-pollution class.
3. Keep second-repo work in the loop, because Cognitive showed that Aionis is useful for continuity but still needs normal repo inspection for domain truth.
