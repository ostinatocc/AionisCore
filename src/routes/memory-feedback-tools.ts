...
import {
  ...
  buildLivePolicyGovernanceRouteContext,
} from "../app/policy-governance-route-context.js";
...
export function registerMemoryFeedbackToolRoutes(args: RegisterMemoryFeedbackToolRoutesArgs) {
  ...
  registerFeedbackPostRoute({
    ...
    withGate: false,
    execute: async (body) => {
      const { parsed, livePolicyContract, liveDerivedPolicy, governanceContract } = await buildLivePolicyGovernanceRouteContext({
        body,
        env,
        embedder,
        embeddedRuntime,
