...
const probeTaskBriefs: Record<string, ProbeTaskBrief> = {
  ...
  external_probe_service_lifecycle_hard: {
    ...
    task_family: "service_publish_validate",
    task_prompt: "Repair the service entrypoint so the verifier-launched process itself remains the service after launcher exit and a fresh shell can validate HTTP health, pid-file JSON, live PID, and lifecycle log evidence.",
    target_files: ["scripts/fixtures/runtime-dogfood/service-lifecycle-hard-server.mjs"],
    next_action: "Inspect the hard service entrypoint, keep the verifier-launched process as the long-lived service process, make it honor --pid-file and --log-file, then validate /healthz plus lifecycle artifacts from a fresh shell.",
    acceptance_checks: [
      "service-lifecycle-hard-verifier --health-url <fresh-shell-endpoint>/healthz --pid-file <path> --log-file <path> --expected-pid <pid>",
      "pid file is JSON with pid equal to expected pid and port equal to health.port",
      "health JSON includes ok=true, pid=<expected-pid>, and port=<service-port>",
      "lifecycle log contains literal service_lifecycle_hard_started",
    ],
    lifecycle_constraints: [
      "must_survive_agent_exit",
      "revalidate_from_fresh_shell",
      ...
      "service_entrypoint_must_not_self_spawn_worker",
    ...
    authority_boundary: [
      ...
      "The expected PID belongs to the verifier-launched service process; spawning a second worker process makes the verifier fail.",
      "The collection harness, not the agent, writes dogfood-run.json.",
    ],
    workflow_steps: [
      "Inspect the service entrypoint and identify required --port, --pid-file, and --log-file arguments.",
      "Do not implement a self-detaching launcher inside the service entrypoint; the harness already launches the entrypoint detached and expects that process PID to be the HTTP server PID.",
      "Implement durable lifecycle evidence before claiming success: write JSON pid metadata with pid and port, return matching pid and port from /healthz, and append the literal service_lifecycle_hard_started marker after listen succeeds.",
      "Probe the endpoint, JSON pid file, log file, and live process from a fresh shell after the launch command returns.",
