...
test("http promote_memory prompt contract is explicitly versioned", () => {
  ...
  assert.equal(contract.transport_contract_version, GOVERNANCE_HTTP_OPENAI_TRANSPORT_CONTRACT_VERSION);
  assert.equal(contract.prompt_version, GOVERNANCE_HTTP_PROMOTE_MEMORY_PROMPT_VERSION);
  assert.equal(contract.user_payload.transport_contract_version, GOVERNANCE_HTTP_OPENAI_TRANSPORT_CONTRACT_VERSION);
...
test("anthropic promote_memory prompt contract is explicitly versioned", () => {
  const contract = buildPromoteMemoryAnthropicHttpPromptContract({
    review_version: "promote_memory_semantic_review_v1",
    operation: "promote_memory",
    requested_target_kind: "workflow",
    requested_target_level: "L2",
  ...
  assert.equal(contract.transport_contract_version, GOVERNANCE_HTTP_ANTHROPIC_TRANSPORT_CONTRACT_VERSION);
  assert.equal(contract.user_payload.transport_contract_version, GOVERNANCE_HTTP_ANTHROPIC_TRANSPORT_CONTRACT_VERSION);
  assert.equal(contract.prompt_version, GOVERNANCE_HTTP_PROMOTE_MEMORY_PROMPT_VERSION);
});

test("anthropic form_pattern prompt contract is explicitly versioned", () => {
  const contract = buildFormPatternAnthropicHttpPromptContract({
    review_version: "form_pattern_semantic_review_v1",
    operation: "form_pattern",
    target_level: "L3",
    source_count: 2,
