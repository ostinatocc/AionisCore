...
test("sidecar supports a wrapper-style task loop without conversational confirmation", async () => {
    ...
    postJson: async (_env, path, body) => {
      ...
      if (path === "/v1/memory/tools/select") {
        return {
          tenant_id: "default",
          scope: "default",
          selection: { selected: "edit", ordered: ["edit", "bash", "test"], preferred: ["edit"] },
  ...
  const ambiguousStep = await sidecar.dispatch({
    request_id: "r3",
    event: {
      ...
      candidates: ["bash", "edit", "test"],
      context: { task_kind: "repair_export" },
      note: "file changed but final validation deferred to task boundary",
    },
  });
  assert.equal(ambiguousStep.ok, true);
  assert.equal((ambiguousStep as any).result.classification, "abstain");
  assert.equal((ambiguousStep as any).result.recorded, false);

  const finalized = await sidecar.dispatch({
    request_id: "r4",
    event: {
